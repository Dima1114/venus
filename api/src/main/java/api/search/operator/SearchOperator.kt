...
enum class SearchOperator(val operator: String,
    ...
    GT("gt", { booleanBuilder, pathBuilder, propertyPath, value, fieldType ->
        when (fieldType) {
            longClass, integerClass -> booleanBuilder.and(pathBuilder.getNumber(propertyPath, longClass).gt(value[0].toLong()))
            doubleClass, floatClass -> booleanBuilder.and(pathBuilder.getNumber(propertyPath, doubleClass).gt(value[0].toDouble()))
        }
    }),
    LT("lt", { booleanBuilder, pathBuilder, propertyPath, value, fieldType ->
        when (fieldType) {
            longClass, integerClass -> booleanBuilder.and(pathBuilder.getNumber(propertyPath, longClass).lt(value[0].toLong()))
            doubleClass, floatClass -> booleanBuilder.and(pathBuilder.getNumber(propertyPath, doubleClass).lt(value[0].toDouble()))
        }
    }),
    GOE("goe", { booleanBuilder, pathBuilder, propertyPath, value, fieldType ->
        when (fieldType) {
            longClass, integerClass -> booleanBuilder.and(pathBuilder.getNumber(propertyPath, longClass).goe(value[0].toLong()))
            doubleClass, floatClass -> booleanBuilder.and(pathBuilder.getNumber(propertyPath, doubleClass).goe(value[0].toDouble()))
        }
    }),
    LOE("loe", { booleanBuilder, pathBuilder, propertyPath, value, fieldType ->
        when (fieldType) {
            longClass, integerClass -> booleanBuilder.and(pathBuilder.getNumber(propertyPath, longClass).loe(value[0].toLong()))
            doubleClass, floatClass -> booleanBuilder.and(pathBuilder.getNumber(propertyPath, doubleClass).loe(value[0].toDouble()))
        }
    }),
